...
temp = input("Path of the Batch File >\t").strip()
...
data = data.replace("%%~", "ckoco").replace("%~", "croco")  # Please don't mind this, I bodged it
...
temp1 = [i.start() for i in finditer("%", data)]
result = [(temp1[i], temp[i] + 8) for i in range(len(temp))]
del temp1
